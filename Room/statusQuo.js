...
Room.prototype.scanRoom = function() {
    if (!this.memory.defcon) this.memory.defcon = 5;
    let currentDefcon = 5;
    if (this.find(FIND_HOSTILE_CREEPS).length) currentDefcon = 4;
    if (this.find(FIND_HOSTILE_CREEPS, {
            filter: c => c.getActiveBodyparts(ATTACK) + c.getActiveBodyparts(RANGED_ATTACK) + c.getActiveBodyparts(HEAL)
        }).length) currentDefcon = 3;
    if (this.getEventLog(true).search("EVENT_ATTACK") > -1) currentDefcon = 1;
    if (this.find(FIND_NUKES).length) currentDefcon = 1;
